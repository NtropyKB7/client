...
- Auth-gated routes set `meta: { requiresAuth: true }` on the parent layout route; a global `router.beforeEach` in `src/app/router/index.js` checks `useAuthStore().isAuthenticated`.

## Gotchas

- `useAuthStore().tryRefresh()` (`src/features/auth/store.js`) and both login handlers in `LoginView.vue` fake a successful login whenever `import.meta.env.DEV` is true OR `VITE_API_BASE_URL` is empty (currently empty in both `.env.development` and `.env.production`, so this also fires on the Vercel deploy). This is because no backend exists yet — don't mistake it for real auth logic. It self-disables once `VITE_API_BASE_URL` is set to a real backend URL.
